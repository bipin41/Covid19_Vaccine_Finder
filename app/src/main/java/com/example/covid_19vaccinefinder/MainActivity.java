...
public class MainActivity extends AppCompatActivity {
    ...
    private EditText etDate, etPin;
    private TextView coRegister, tvResults;
    ...
    private RadioButton rbPin_code, rbDistrict;
    private Spinner spState, spDistrict;
    private Button btnCheck;
    private RelativeLayout pinLayout, districtLayout;
    private RadioGroup radioGroup;
    private String month, day, year, pin_code, districtId, date = null;

    private ArrayList<dataFile> stateData, AndamanandNicobarIslands, AndhraPradesh, ArunachalPradesh, Assam, Bihar, Chandigarh, Chhattisgarh, DadraandNagarHaveli,
            DamanandDiu, Delhi, Goa, Gujarat, Haryana, HimachalPradesh, JammuandKashmir, Jharkhand, Karnataka, Kerala,
            Ladakh, Lakshadweep, MadhyaPradesh, Maharashtra, Manipur, Meghalaya, Mizoram, Nagaland, Odisha, Puducherry,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String pin = sp.getString("pincode", "");
        etPin.setText(pin);

        AnimatedVectorDrawableCompat avd = AnimatedVectorDrawableCompat.create(this, R.drawable.avd_anim);
        ivDown.setImageDrawable(avd);
    }
    ...
    private void notification() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            CharSequence name = "Vaccine Finder";
            String message = "Vaccine Doses Available in your area. Check it out >";
            String msg = "હેલો મિત્રો! તમે બધા કેમ છો? તો આજે અમે તમારા માટે ગુજરાતી શાયરી, ગુજરાતી રોમેન્ટિક શાયરી, લવ શાયરી ગુજરાતી, ગુજરાતી શાયરી લવ રોમેન્ટિક, ગુજરાતીમાં લવ સ્ટેટસ, ગુજરાતી લવ નામનો એક નવો લેખ લાવ્યા છીએ, તેથી જો તમે ગૂગલ પર આ લેખના નામની શોધ કરો તો તમે અમારા આ લેખનો મોટો સંગ્રહ જોવા માટે મેળવો. તેથી તમે આ પોસ્ટ તમારા મિત્રો, રિલેટિવ્સને શેર કરી શકો છો. જો તમને અમારો લેખ ગમે છે, તો પછી તમે આ લેખના બધા વિચારો વાંચશો અને આ વિચારો ફેસબુક પર પણ શેર કરી શકશો. તો ચાલો સમય બગાડ્યા વિના આ લેખ શરૂ કરીએ.";
            ...
            NotificationCompat.Builder builder = new NotificationCompat.Builder(this, CHANNEL_ID)
                    .setSmallIcon(R.drawable.find)
                    .setContentText(message)
                    //.setStyle(new  NotificationCompat.BigPictureStyle().bigPicture(BitmapFactory.decodeResource(getResources(),R.drawable.bipin)))
    ...
    //intialize the state and district spinner here.
        ...
        spState.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                spDistrict.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                    @Override
                    public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                        dataFile selectedDistrict = (dataFile) adapterView.getSelectedItem();
                        districtId = String.valueOf(selectedDistrict.getId());
                    }
    ...
    //here get the data from CO-WIN API and divide into required category.
    private void populateResultView(String date, String pin_code, String districtId) {
        String url = null;
        list = new ArrayList<>();
        if (pin_code != null)
    ...
    //intialize recyclerView for vaccine data.
    private void initialize_recycler_view() {
        rvResult.setNestedScrollingEnabled(false);
        rvResult.setHasFixedSize(false);
        RecyclerView.LayoutManager result_list_layout_manager = new LinearLayoutManager(MainActivity.this, RecyclerView.VERTICAL, false);
        rvResult.setLayoutManager(result_list_layout_manager);
        resultListAdapter adapter = new resultListAdapter(getApplicationContext(), list);
        rvResult.setAdapter(adapter);
    }

    //all the click events define here.
    private void onClickEvents() {
        ...
        rbPin_code.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
        ...
        rbDistrict.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b) {
        ...
        coRegister.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(MainActivity.this, SlotBooking.class);
                startActivity(i);
            }
        });
        ...
        btnCheck.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Animatable animatable = (Animatable) ivDown.getDrawable();
                ivDown.setVisibility(View.VISIBLE);
                animatable.start();
                animatable.isRunning();

                if (((etDate.getText().toString().isEmpty()) && (radioGroup.getCheckedRadioButtonId() == -1)) || (radioGroup.getCheckedRadioButtonId() == -1)) {
                    if ((etDate.getText().toString().isEmpty()) && (radioGroup.getCheckedRadioButtonId() == -1))
                ...
                if (rbPin_code.isChecked()) {

                    if ((etDate.getText().toString().isEmpty()) || (etPin.getText().toString().isEmpty())) {
                        if (etDate.getText().toString().isEmpty())
                            etDate.setError("Select Date");

                        if (etPin.getText().toString().isEmpty())
                            etPin.setError("Enter Pin-code");
                    } else {
                        date = etDate.getText().toString();
                        pin_code = etPin.getText().toString();
                        populateResultView(date, pin_code, null);
                        //progressDialog.dismiss();
    ...
    //add all the districts to state vise in the arrayList.
    ...
    //add all the states in arrayList.
